                ...
                {rows.map((item)=>(
                    loadMore == false ?( (item.id)>120 && (item.id)<128?(
                        ...
                        <div className="newsText">
                            <div>
                                <Link to='/showblog'><button className="hideBtn" onClick={()=>handleId(item.id)}><span className="fitnessTextTitle">{item.title}</span></button></Link>                               
                                <br />
                    ...
                    <div>
                        <button className="loadMoreFitness" onClick={()=>setLoadMore(handleLoadMore)}>{loadmorebtn}</button>  
                    </div>
                </div>
                            ...
                            {rows.map((item)=>(
                                (item.id) == 141 ? (
                                    <div className="topFitnessPostsContent1">
                                        ...
                                        <Link to='/showblog'><button className="hideBtn" onClick={()=>handleId(item.id)}><span className="topFitnessPostsTextTitle">{item.title}</span></button></Link>
                                        <div className="lowerNewsText">
                                            <span className='category'>{item.category}</span>
                                            <span className='date'> {item.date}</span>
                                        </div>
                                    ...
                                    <div className="topFitnessPostsContent2">
                                        <img src={item.image} className="topFitnessPostsImage2"/>
                                            <div className="alignColumn">
                                                <Link to='/showblog'><button className="hideBtn" onClick={()=>handleId(item.id)}><span className="topFitnessPostsTextTitle2">{item.title}</span></button></Link>
